...
#m cxxRecordDecl(                                                            \
    ...
    forEach(eachOf(                                                          \
        ...
        fieldDecl(hasType(cxxRecordDecl(hasName("sc_inout"))))               \
            .bind("sc_inout"),                                               \
        fieldDecl(hasType(cxxRecordDecl(hasName("sc_signal"))))              \
            .bind("sc_signal"))))                                            \
    .bind("sc_module")
...
#m cxxRecordDecl(                                                            \
    isExpansionInMainFile(), isDerivedFrom(hasName("::sc_core::sc_module")), \
    unless(isDerivedFrom(matchesName("sc_event_queue"))),                    \
    forEach(fieldDecl(unless(hasType(cxxRecordDecl(matchesName("sc*")))))    \
                .bind("non_sc")))
...
#
#m cxxRecordDecl(isDerivedFrom(hasName("sc_module")),                        \
...
#m cxxRecordDecl(isExpansionInMainFile(),                        \
                 isDerivedFrom(hasName("::sc_core::sc_module")), \
                 hasDescendant(cxxConstructorDecl().bind("constructor")))
...
#m cxxMethodDecl(isExpansionInMainFile())
        m declStmt(hasAncestor(cxxRecordDecl(
                       isDerivedFrom(hasName("::sc_core::sc_module")))),
                   hasAncestor(cxxMethodDecl()),
                   hasDescendant(varDecl(unless(
                       hasType(cxxRecordDecl(hasName("sc_process_handle")))))))
