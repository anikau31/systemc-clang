...
class FieldMatcher : public MatchFinder::MatchCallback {
 public:
  void registerMatchers(MatchFinder &finder) {
    ...
    auto match_in_ports = cxxRecordDecl(forEachDescendant(
        //              declStmt().bind("sc_in")
        declStmt(hasAncestor(cxxRecordDecl(
                     isDerivedFrom(hasName("::sc_core::sc_module")))),
  ...
  virtual void run(const MatchFinder::MatchResult &result) {
    ...
    if (auto field_decl =
            const_cast<Decl *>(result.Nodes.getNodeAs<Decl>("sc_in"))) {
      cout << " Found an Decl: ";
    }

    if (auto field_decl =
            const_cast<DeclStmt *>(result.Nodes.getNodeAs<DeclStmt>("sc_in"))) {
      field_decl->dump();
