...
def validate_args(args):
    ...
    if use_cpp ^ has_cpp:
        raise ValueError(
            "Action '{}' and --cpp is not compatible, --cpp must be present exactly when the action is one of cpp-to-v or cpp-to-hdl".format(
                args.action
            )
        )
    if use_hdl ^ has_hdl:
        raise ValueError(
            "Action '{}' and --hdl is not compatible, --hdl must be present exactly when the action is hdl-to-v".format(
                args.action
            )
        )
    ...
    if has_golden_interm and not gen_hdl:
        raise ValueError(
            "Action '{}' and --golden-intermediate is not compatible, --golden-intermediate may only be present when the action is one of cpp-to-v or cpp-to-hdl".format(
                args.action
            )
        )
    if has_golden_verilog and not gen_verilog:
        raise ValueError(
            "Action '{}' and --golden-verilog is not compatible, --golden-verilog may only be present when the action is one of cpp-to-v or hdl-to-v".format(
                args.action
            )
        )
...
def cpp_to_hdl(args):
    ...
    extra_options = list(
        chain.from_iterable(("-I", inc_path) for inc_path in args.include_path)
    )
    output_dir = get_output_folder(args)
    print("cmd: ")
    target = sysc_clang.invoke_sysc(
        [
            "-hdl-file-out",
    ...
    if args.verbose:
        print(
            "The _hdl.txt file is written to: {}".format(os.path.abspath(destination))
        )
...
def check_hdl(args):
    ...
    if is_diff:
        ...
        print(
            "* HDL is the same as specified golden input {}".format(
                args.golden_intermediate
            )
        )
    output_dir = get_output_folder(args)
    if is_diff:
...
def check_verilog(args):
    ...
    if is_diff:
        ...
        print(
            "* Verilog is the same as specified golden input {}".format(
                args.golden_verilog
            )
        )
    output_dir = get_output_folder(args)
...
def main():
    parser = argparse.ArgumentParser(
        "A tool for running and comparing against a golden standard"
    )
    parser.add_argument(
        "action",
        choices=["cpp-to-hdl", "hdl-to-v", "cpp-to-v"],
        help="Action to perfrom",
    )
    parser.add_argument("--cpp", help="The SystemC C++ file to convert")
    parser.add_argument("--hdl", help="The _hdl.txt file to convert")
    parser.add_argument(
        "--golden-intermediate", help="The _hdl.txt file to compare against"
    )
    ...
    parser.add_argument(
        ...
        default=list(),
        nargs="*",
        help="Include path (-I) option for the systemc-clang command",
    )
    parser.add_argument(
        "--output-dir",
        help="The output folder to store the results. "
    ...
    parser.add_argument(
        ...
        default=False,
        action="store_true",
        help="Whether show the output of the called tools (systemc-clang and hcode2verilog.py)",
    )
    parser.add_argument(
        "--no-ts",
        default=False,
        action="store_true",
        help="Do not create time-stamped folder",
    )
    parser.add_argument(
        "--force",
        default=False,
        action="store_true",
        help="Overwrite content if necessary",
    )
    ...
    flow_flags = {
        ...
        check_hdl: args.action in ["cpp-to-v", "cpp-to-hdl"]
        and args.golden_intermediate is not None,
        check_verilog: args.action in ["cpp-to-v", "hdl-to-v"]
        and args.golden_verilog is not None,
