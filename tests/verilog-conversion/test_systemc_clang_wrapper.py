...
def test_syntax_error(tmp_path, syntax_error_cpp, default_params, clang_args_params):
    target_path = tmp_path / 'simple.cpp'
    with open(target_path, 'w') as f:
        f.writelines(syntax_error_cpp)

    with pytest.raises(sysc_clang.SystemCClangFatalError):
        target = sysc_clang.invoke_sysc([str(target_path), '--debug', '--'] + default_params + clang_args_params)


def test_generate_file(tmp_path, simple_add_cpp, default_params, clang_args_params):
    target_path = tmp_path / 'simple.cpp'
    with open(target_path, 'w') as f:
