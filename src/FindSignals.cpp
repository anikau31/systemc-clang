...
bool FindSignals::VisitFieldDecl(FieldDecl *fd) {
  ...
  if (IdentifierInfo *info = fd->getIdentifier()) {
    ...
    signalcontainer_map_.insert(
        FindSignals::signalPairType(fd->getNameAsString(), sc));
  }
  return true;
}

FindSignals::signalMapType FindSignals::getSignals() const {
  return signalcontainer_map_;
}
