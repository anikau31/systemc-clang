...
namespace scpar {
  ...
  public:
    ...
    FindTemplateTypes(const FindTemplateTypes& rhs)  {
      copy(rhs._templateTypes.begin(), rhs._templateTypes.end(), back_inserter(_templateTypes));
    }
...
//            _os << "\n###### Type: " << q.getAsString() << " \n" ;
    ...
    bool VisitIntegerLiteral(IntegerLiteral *l) {
      ...
      _templateTypes.push_back(pair<string, const Type*> (l->getValue().toString(10,true), l->getType().getTypePtr()));
