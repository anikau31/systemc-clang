...
bool SystemCConsumer::fire() {
  ...
  for (auto const &element : found_module_declarations) {
    ...
    module_declaration->addPorts( module_declaration_handler.getFields("sc_in"),
    "sc_in" ); module_declaration->addPorts(
    module_declaration_handler.getFields("sc_out"), "sc_out" );
    module_declaration->addPorts (
    module_declaration_handler.getFields("sc_inout"), "sc_inout" );
    module_declaration->addPorts (
    module_declaration_handler.getFields("others"), "others" );
    module_declaration->addPorts (
    module_declaration_handler.getFields("sc_signal"), "others");
    ...
    */
    ...
    //
    FindEntryFunctions find_entries{module_declaration->getModuleClassDecl(),
                                    os_};
    ...
    for (size_t i{0}; i < entry_functions->size(); ++i) {
      ...
      FindSensitivity find_sensitivity{constructor.returnConstructorStmt(),
                                       os_};
  ...
  // So, the CXXRecordDecl to be passed to the next stage should be topModule if
  // found.
