...
GPUMacro::GPUMacro(int blockIdx, int blockIdy, int blockIdz, int threadIdx,
    ...
    : _blockIdx(blockIdx), _blockIdy(blockIdy), _blockIdz(blockIdz),
      _threadIdx(threadIdx), _threadIdy(threadIdy), _threadIdz(threadIdz),
      _gpuTime(gpuTime), _cpuTime(cpuTime), _gpuFit(false) {}

GPUMacro::GPUMacro()
    : _blockIdx(1), _blockIdy(1), _blockIdz(1), _threadIdx(1), _threadIdy(1),
      _threadIdz(1), _gpuTime(0), _cpuTime(0), _gpuFit(false) {}

GPUMacro::~GPUMacro() {}

void GPUMacro::addGPUFit() { _gpuFit = true; }

void GPUMacro::denyGPUFit() { _gpuFit = false; }

bool GPUMacro::isGPUFit() { return _gpuFit; }

int GPUMacro::getBlockIdx() { return _blockIdx; }

int GPUMacro::getBlockIdy() { return _blockIdy; }

int GPUMacro::getBlockIdz() { return _blockIdz; }

int GPUMacro::getThreadIdx() { return _threadIdx; }

int GPUMacro::getThreadIdy() { return _threadIdx; }

int GPUMacro::getThreadIdz() { return _threadIdz; }

int GPUMacro::getCPUTime() { return _cpuTime; }

int GPUMacro::getGPUTime() { return _gpuTime; }
...
///////////////////////////////////////////////////////
FindGPUMacro::FindGPUMacro(CXXMethodDecl *entryFunction, int instanceNum,
                           llvm::raw_ostream &os)
    : _entryFunction(entryFunction), _instanceNum(instanceNum), _os(os) {
...
bool FindGPUMacro::VisitForStmt(ForStmt *fstmt) {
  ...
  int tx = 1, ty = 1, tz = 1, bx = 1, by = 1, bz = 1, gpu_time = 0,
      cpu_time = 0, instanceNum = 0;
  for (Stmt::child_iterator it = body->child_begin(), eit = body->child_end();
    ...
    if (DeclStmt *ds = dyn_cast<DeclStmt>(s)) {
      if (VarDecl *vd = dyn_cast<VarDecl>(ds->getSingleDecl())) {
        string className = vd->getTypeSourceInfo()
                               ->getType()
                               .getBaseTypeIdentifier()
                               ->getName();
        if (className == "profile_time") {
          if (CXXConstructExpr *ce =
                  dyn_cast<CXXConstructExpr>(vd->getInit()->IgnoreImpCasts())) {
            if (MaterializeTemporaryExpr *me =
                    dyn_cast<MaterializeTemporaryExpr>(
                        ce->getArg(0)->IgnoreImpCasts())) {
              if (CXXTemporaryObjectExpr *co = dyn_cast<CXXTemporaryObjectExpr>(
                      me->GetTemporaryExpr()->IgnoreImpCasts())) {
    ...
    //instanceNum : " <<_instanceNum<<" " <<instanceNum;
    if (tx && ty && tz && bx && by && bz && gpu_time && cpu_time &&
      ...
      //_os <<"\n for stmt : " <<fstmt;
      forStmtInstanceIdPairType forStmtInstanceId =
          make_pair(_instanceNum, fstmt);
      _forStmtGPUMacroMap.insert(
          forStmtGPUMacroPairType(forStmtInstanceId, gm));
      break;
    }
  }
  return true;
...
void FindGPUMacro::dump() {
  _os << "\n For Stmt GPU Macro";
  for (forStmtGPUMacroMapType::iterator it = _forStmtGPUMacroMap.begin(),
                                        eit = _forStmtGPUMacroMap.end();
       it != eit; it++) {
...
void FindGPUMacro::analyze_array_base(Expr *base, bool isLHS) {
  if (CastExpr *cast = dyn_cast<CastExpr>(base)) {
    ...
    if (DeclRefExpr *declRef = dyn_cast<DeclRefExpr>(subExpr)) {
      ...
      _os << "Type not a DeclRefExpr" << cast->getStmtClassName() << "\n";
    }
  } else {
    _os << "Type not a CastExpr" << cast->getStmtClassName() << "\n";
  }
}
...
void FindGPUMacro::analyze_lhs(Expr *expr) {
  if (ArraySubscriptExpr *array = dyn_cast<ArraySubscriptExpr>(expr)) {
    ...
    if (binOp->isAssignmentOp()) {
      ...
      _os << "Operation without effects on LHS: " << binOp->getStmtClassName()
          << "\n";
    }
  }
}
...
void FindGPUMacro::analyze_expr(Expr *expr) {
  if (BinaryOperator *binOp = dyn_cast<BinaryOperator>(expr)) {
    if (binOp->isAssignmentOp()) {
      ...
      _os << "Operation without effects: " << binOp->getStmtClassName() << "\n";
    }
  }
}
...
void FindGPUMacro::analyze_data_struct(Stmt *stmtList) {
  ...
  for (std::set<ValueDecl *>::iterator itr = lhs_decls.begin();
       itr != lhs_decls.end(); ++itr) {
    _os << "LHS ValueDecl AST Node: " << *itr << ", Name: '"
        << (*itr)->getNameAsString() << "', ";
    analyze_value_decl(*itr);
  }

  for (std::set<ValueDecl *>::iterator itr = rhs_decls.begin();
       itr != rhs_decls.end(); ++itr) {
    _os << "RHS ValueDecl AST Node: " << *itr << ", Name: '"
        << (*itr)->getNameAsString() << "', ";
    analyze_value_decl(*itr);
  }
