...
void FindPorts::dump() {
  ...
  for (portType::iterator mit=_inPorts.begin(),mitend=_inPorts.end(); mit!=mitend;
      mit++) {
    _os << "\n:>> " << mit->first;
  ...
  for (portType::iterator mit=_outPorts.begin(),mitend=_outPorts.end(); mit!=mitend;
      mit++) {
    _os << "\n:>> " << mit->first;
    (mit->second)->printTemplateArguments(_os);
  }
  ...
  for (portType::iterator mit=_inoutPorts.begin(),mitend=_inoutPorts.end(); mit!=mitend;
      mit++) {
    _os << "\n:>> " << mit->first;
    (mit->second)->printTemplateArguments(_os);
  }
