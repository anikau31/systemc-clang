  ...
  case 3: {
      /// Check if name is the same as _entryFunction.
    	...
    	if (md->getNameAsString() == _entryFunctions.at(i)->getName()) {     					
				EntryFunctionContainer *tmp = _entryFunctions.at(i);
				_entryMethodDecl = md;
...
void FindEntryFunctions::dump() {
  ...
  for (unsigned int i = 0; i < _entryFunctions.size(); i++) {
    ...
    case THREAD: _os << " SC_THREAD\n";
      break;
    case METHOD: _os << " SC_METHOD\n";
      break;
    case CTHREAD: _os << " SC_CTHREAD\n";
      break;
    default: _os << " NONE\n";
      break;
    }
