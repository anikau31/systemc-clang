...
bool FindSensitivity::VisitMemberExpr(MemberExpr* e) {
  ...
  if (!_foundSensitiveNode) {
    ...
    if ((q.getAsString() != "class sc_core::sc_sensitive") || (memberName != "sensitive")) {
      return true;
    }
  }
  ...
  /// Is it a port type
  if (!(ait->first == "sc_in" || ait->first == "sc_out" || ait->first == "sc_inout") ) {
    return true;
  }
...
void FindSensitivity::dump() {
  ...
  for (senseMapType::iterator mit=_sensitivePorts.begin(), mitend=_sensitivePorts.end(); mit!=mitend; mit++) {
