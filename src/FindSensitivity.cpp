...
FindSensitivity::FindSensitivity(Stmt *s, llvm::raw_ostream &os)
    : _os(os), _foundSensitiveNode(false) {
...
bool FindSensitivity::VisitMemberExpr(MemberExpr *e) {
  ...
  /// It is a port type. Now we can do whatever we need to do to get the port
  /// name.
  //  _os << "@@@@@: " << memberType.getAsString() << ", " << ait->first <<
  //  "\n";

  _sensitivePorts.insert(kvType(e->getMemberDecl()->getNameAsString(), e));
  //  _reflectionContainer->setChildEntries(new
  //  ReflectionContainerClass(e->getMemberDecl()->getNameAsString(), e));
  return true;
}
...
void FindSensitivity::dump() {
  ...
  for (senseMapType::iterator mit = _sensitivePorts.begin(),
                              mitend = _sensitivePorts.end();
       mit != mitend; mit++) {
