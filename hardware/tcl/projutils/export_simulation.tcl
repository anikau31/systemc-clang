...
namespace eval ::tclapp::xilinx::projutils {
  namespace export ext_export_simulation
}

namespace eval ::tclapp::xilinx::projutils {
proc ext_export_simulation {args} {
  ...
  # Customized options
  # hopefully this will not collide with other options
  variable l_extra_xvlog_opts ""
  variable l_extra_xelab_opts ""
  ...
  for {set i 0} {$i < [llength $args]} {incr i} {
    ...
    switch -regexp -- $option {
      ...
      "-extra_xvlog_opt"          { incr i;set l_extra_xvlog_opts                [lindex $args $i];set a_sim_vars(b_extra_xvlog_opts_specified)          1 }
      "-extra_xelab_opt"          { incr i;set l_extra_xelab_opts                [lindex $args $i];set a_sim_vars(b_extra_xelab_opts_specified)          1 }
...
proc xps_write_multi_step { simulator fh_unix launch_dir srcs_dir } {
  ...
  switch $simulator { 
    "xsim" {
      ...
      if { $a_sim_vars(b_contain_systemc_sources) } {
        puts $fh_unix "  xsc \$xsc_opts -f xsc.prj $redirect"
      }
...
proc xps_write_xelab_cmdline { fh_unix launch_dir } {
  ...
  if { $a_sim_vars(b_contain_systemc_sources) } {
    set args [list]
    lappend args "--shared -lib $a_sim_vars(default_lib)" 
    foreach lib [xcs_get_sc_libs] {
      lappend args "-lib $lib"
    }
  ...
  if { ($a_sim_vars(b_contain_systemc_sources)) || ($a_sim_vars(b_contain_systemc_headers)) } {
  ...
  if { $a_sim_vars(b_contain_systemc_sources) } {
    if {$::tcl_platform(platform) == "unix"} {
      lappend args "-sv_root \".\" -sc_lib libdpi"
    }
  }
  ...
  # extra xelab parameters
  variable l_extra_xelab_opts
  #
  if { $a_sim_vars(b_extra_xelab_opts_specified) } {
    lappend arg "$l_extra_xelab_opts"
  }
...
proc xps_write_xsim_cmdline { fh_unix dir } {
  ...
  if { $a_sim_vars(b_contain_systemc_sources) } {
    puts $fh_unix "  xv_lib_path=\"$::env(RDI_LIBDIR)\""
    puts $fh_unix "  export LD_LIBRARY_PATH=\$PWD:\$xv_lib_path:\$LD_LIBRARY_PATH"
  }
    ...
    switch -regexp -- $simulator {
      "xsim" {
        ...
        if { [xcs_contains_verilog $a_sim_vars(l_design_files)] } {
          ...
          if { $a_sim_vars(b_extra_xvlog_opts_specified) } {
            lappend arg_list "$l_extra_xvlog_opts"
          }
        ...
        if { $a_sim_vars(b_contain_systemc_sources) } {
          set arg_list [list]
          xps_append_config_opts arg_list "xsim" "xsc"
          ...
          lappend arg_list "-f $sc_filename"
